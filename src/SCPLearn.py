...
def main():
    ...
    try:
        opts, files = getopt.gnu_getopt(sys.argv[1:], "hd:n:r:s:m:o:p:vVuw:lt:",
    ...
    if fileType == "nii":
        ...
        cmdArray=['ComputeROIAverages.py','-d',dataFile,'-A',maskFile,'-o',outDir]
    ...
    cmdArray=['SCPLearnFromMatFiles',matlist,numberOfSCPs,pruning,sparsity,outprefix,levels,str(verbose)]
    execCmd(cmdArray,verbose=1,simulate=False)
    ...
    for kk in range(nBasis):
        ...
        cmdArray=['replaceLabels_nib.py','-d',SCPBasisFile,'-A',maskFile,'-H',str(kk+1),'-p',scp_nifti,'-o',outDir]
        ret=execCmd(cmdArray,verbose=1,simulate=False)
        if ret[0] != 0:
            cryandexit("Error in generating SCPs, ", scp_nifti," not found")
